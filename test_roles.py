...
async def test_roles(pool_handler, wallet_handler, get_default_trustee):
    ...
    # Trustee demotes new Steward
    res3 = await nym_helper(pool_handler, wallet_handler, trustee_did, new_steward_did, None, None,
                            '')
    assert res3['op'] == 'REPLY'
    # New Steward try to add TA2 after demotion - should fail
    res4 = await nym_helper(pool_handler, wallet_handler, new_steward_did, anchor2_did, anchor2_vk, None,
                            'TRUST_ANCHOR')
    assert res4['op'] == 'REJECT'
    # Trustee promotes new Steward back
    res5 = await nym_helper(pool_handler, wallet_handler, trustee_did, new_steward_did, None, None,
                            'STEWARD')
    assert res5['op'] == 'REPLY'
    # New Steward try to add TA2 after promotion
    res6 = await nym_helper(pool_handler, wallet_handler, new_steward_did, anchor2_did, anchor2_vk, None,
                            'TRUST_ANCHOR')
