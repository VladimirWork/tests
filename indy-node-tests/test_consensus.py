...
async def test_consensus_restore_after_f_plus_one(pool_handler, wallet_handler,
    ...
    # 5/7 online - can w+r
    outputs = [host.run('systemctl stop indy-node-tests') for host in hosts[-2:]]
    assert outputs
    await send_and_get_nym(pool_handler, wallet_handler, trustee_did, did2)
    # 4/7 online - can r only
    hosts[4].run('systemctl stop indy-node-tests')
    ...
    hosts[3].run('systemctl stop indy-node-tests')
    with pytest.raises(IndyError, match='Consensus is impossible'):
    ...
    # 5/7 online - can w+r
    outputs = [host.run('systemctl start indy-node-tests') for host in hosts[3:5]]
    assert outputs
    time.sleep(30)
    await send_and_get_nym(pool_handler, wallet_handler, trustee_did, did3)
    # 7/7 online - can w+r
    outputs = [host.run('systemctl start indy-node-tests') for host in hosts[-2:]]
    assert outputs
...
async def test_consensus_state_proof_reading(pool_handler, wallet_handler,
    ...
    time.sleep(5)
    # Stop all except 1
    outputs = [host.run('systemctl stop indy-node-tests') for host in hosts[1:]]
    assert outputs
    time.sleep(5)
    res = await get_nym_helper(pool_handler, wallet_handler, trustee_did, did1)
    assert res['result']['seqNo'] is not None
    # Stop the last one
    hosts[0].run('systemctl stop indy-node-tests')
    # Start all
    outputs = [host.run('systemctl start indy-node-tests') for host in hosts]
    assert outputs
    time.sleep(30)
    await send_and_get_nym(pool_handler, wallet_handler, trustee_did, did2)
...
async def test_consensus_n_and_f_changing(pool_handler, wallet_handler, get_default_trustee):
    ...
    outputs = [host.run('systemctl stop indy-node-tests') for host in temp_hosts[-2:]]
    assert outputs
    with pytest.raises(IndyError, match='Consensus is impossible'):
        await nym_helper(pool_handler, wallet_handler, trustee_did, did1, None, None, None)
    outputs = [host.run('systemctl start indy-node-tests') for host in temp_hosts[-2:]]
    assert outputs
    time.sleep(30)
